...
import 'package:project_flutter/movie/models/movie_detail_model.dart';
import 'package:project_flutter/movie/models/movie_model.dart';
import 'package:project_flutter/movie/models/movie_video_model.dart';
...
class MovieRepositoryImpl implements MovieRepository {
  ...
  @override
  Future<Either<String, List<MovieCastModel>>> getCast(
    ...
    try {
      ...
      if (result.statusCode == 200 && result.data != null) {
        final List castJson = result.data["cast"] ?? [];

        final casts = castJson.map((e) => MovieCastModel.fromMap(e)).toList();

        return Right(casts);
      }

      return const Left('Error get movie cast');
    } on DioError catch (e) {
      if (e.response != null) {
        return Left(e.response.toString());
      }
      return const Left('Another error on get movie cast');
    }
  }
}
